...
from data_portal.wrs_06_url import WRS_06   
...
class wrs06Ser :
    ...
    def querySer(self):
        ...
        cfig = config('WRS_06','waterLevel_fcltyList_url', 'waterLevel_service_key')
        #query 조합하는 클래스
        query = WRS_06(cfig.reUrl(), cfig.reSerKey(), self.divCode).setUrl()
        print('쿼리날리기')
        try :
            re = Request(query)
            res = re.doReq()
